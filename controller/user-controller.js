...
exports.forgetPassword = async (req, res) => {
  try {
    ...
    if (err.status)
      return res
        .status(err.status)
        .json(helpers.sendError(err.message, err.status));
  }
};
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json(helpers.sendSuccess("Password reset was successful", 200));
  } catch (error) {
    if (error.status)
      return res
        .status(error.status)
...
exports.updateAccount = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json(helpers.sendSuccess("profile has been successfully updated", 200));
  } catch (error) {
    if (error.status)
      return res
        .status(error.status)
...
exports.bookDriver = async (req, res) => {
  try {
    ...
    const bookDriverservices = await driverServices.bookDriver(
      userID,
      driverID
    );

    //  bookDriverservices.
    helpers.sendSuccess("Your ride has been has been booked successfuly");
  } catch (error) {
    if (error.status)
      return res
        .status(error.status)
...
exports.deleteUser = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(helpers.sendSuccess("user successfully deleted", 200));
  } catch (error) {
    if (error.status)
      return res
        .status(error.status)
